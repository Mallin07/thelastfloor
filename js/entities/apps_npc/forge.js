...
// ----------------------------
export function canForge(state, recipeId){
...
// ----------------------------
export function forgeItem(state, recipeId){
  const recipe = FORGE_RECIPES?.[recipeId];
  if (!recipe){
  ...
  const check = canForge(state, recipeId);
  if (!check.ok){
  ...
  logOk(
    `Forjaste: ${outQty} × ${firstItem.icon ?? "⚒️"} ${firstItem.name} (-${cost} oro).`
  );
