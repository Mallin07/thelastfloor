...
export function canCarpenterCraft(state, recipeId) {
  const r = CARP_RECIPES?.[recipeId];
...
export function carpenterCraftRecipe(state, recipeId) {
  const r = CARP_RECIPES?.[recipeId];
  if (!r) {
  ...
  const check = canCarpenterCraft(state, recipeId);
  if (!check.ok) {
  ...
  onQuestEvent(state, "craft", { recipeId, kind:"carpenter" });
  logOk(`Has creado: ${r.name}`);

  playSound("craft_wood");
  
  return true;
}

// helpers (idénticos a mago)
